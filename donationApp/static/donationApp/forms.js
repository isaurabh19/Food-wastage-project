function Formset(element) {
  ...
  */
  ...
  var initialForms = element.querySelector('[name$=form-INITIAL_FORMS]');
  var totalForms = element.querySelector('[name$=form-TOTAL_FORMS]');
//  var prefix = initialForms.name.replace(/INITIAL_FORM_COUNT$/, '');
  ...
  function renumberForm(form, oldValue, newValue) {
    var matchValue = oldValue.toString()
    var match = new RegExp(matchValue);
    var replace = newValue.toString();
...
//new Formset(document.querySelector('#formset'));